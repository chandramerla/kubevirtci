...
(
    ...
    if [ "${CI}" == "true" ] && [ -f $conformance_config ]; then
        if [ "$RUN_KUBEVIRT_CONFORMANCE" == "true" ]; then
            ...
            # TODO: Uncomment above line and remove below lines (workaround) once we get conformance-s390x.yaml published
            conformance_yaml="$(pwd)/$(mktemp conformance_XXXXXXXX.yaml)"
            curl -o "$conformance_yaml" -s -L -f https://storage.googleapis.com/kubevirt-prow/devel/nightly/release/kubevirt/kubevirt/${LATEST}/conformance.yaml
            function cleanup() {
                rm -f "$conformance_yaml"
            }
            trap cleanup EXIT SIGTERM

            if [[ $(uname -m) == *s390x* ]]; then
                sed -i 's|\(image: quay.io/kubevirt/conformance:\)\(.*\)|\1\2-s390x|' $conformance_yaml
            fi
            export SONOBUOY_EXTRA_ARGS="--plugin $conformance_yaml"
