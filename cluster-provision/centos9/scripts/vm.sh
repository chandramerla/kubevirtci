...
ssh -vvv -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ${VM_USER}@192.168.66.1${n} -i ${VM_USER_SSH_KEY} -p 22 -q \$@
EOL
...
if [[ -n "$QEMU_MONITOR_ARGS" ]]; then
  echo "$QEMU_MONITOR_ARGS" | socat - UNIX-CONNECT:/tmp/qemu-monitor.sock
fi
