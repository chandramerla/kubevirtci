...
#!/bin/bash
set -ex
dockerize -wait tcp://192.168.66.1${n}:22 -timeout 3600s &>/dev/null
...
qemu-system-s390x -enable-kvm -drive format=qcow2,file=box.qcow2,if=virtio,cache=unsafe -machine s390-ccw-virtio -device virtio-net-ccw,netdev=network0,mac=52:55:00:d1:55:${n} -netdev tap,id=network0,ifname=tap01,script=no,downscript=no -device virtio-rng-pci -vnc :01 -cpu host -m 3096M -smp 2 -serial pty -uuid $(cat /proc/sys/kernel/random/uuid)  ${QEMU_ARGS}  >"$qemu_log" 2>&1
